...
export const PostHandler: FastifyPluginCallback = (fastify, _, done) => {
    fastify.get("/post", async (req, res) => {
        const query = req.query as any;

        if (!query.title || !query.description)
            throw new Error("No title or description");
        ...
        svg.addChild(
            new Element("text", {
                ...
                y: 300,
                "font-size":
                    title.length <= 20 ? 64 : title.length <= 26 ? 56 : 46
